...
class MRHATradingSystem:
    ...
    def implement_trading_logic(self):
        ...
        if last_signal == 1:
            self.send_telegram_message(f"MRHA Fibonacci - BUY Signal for {self.symbol} at {signals.index[-1]}: Price {self.mrha_data['mh_close'].iloc[-1]}")
        elif last_signal == -1:
            self.send_telegram_message(f"MRHA Fibonacci - SELL Signal for {self.symbol} at {signals.index[-1]}: Price {self.mrha_data['mh_close'].iloc[-1]}")
